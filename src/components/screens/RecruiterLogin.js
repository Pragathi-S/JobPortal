  ...
  const loginHandler = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "http://localhost:5000/api/auth/recruiterlogin",
        { email, password },
        config
      );
    ...
    <div className="login-screen">
      ...
      <form onSubmit={loginHandler} className="login-screen_form">
        ...
        <span className="login-screen_subtext">
          Don't have an account? <Link className="login-screen_sublink" to="/recruiterregister">Register</Link>
        </span>
      </form>
    </div>
  );
